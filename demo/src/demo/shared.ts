//import { Key, Reducer } from "solid-js";
import { ContentLocation, PopoverProps } from "solid-tiny-popover";
...
export type Action<K extends keyof ControlsState> = K extends Keys["padding"]
  ...
  ? {
      type: Keys["contentLocationEnabled"];
      payload: ControlsState["contentLocationEnabled"];
    }
  : K extends Keys["containerClassName"]
  ? {
      type: Keys["containerClassName"];
      payload: ControlsState["containerClassName"];
    }
  : never;

//// will be removed ASAP
export const reducer: Reducer<ControlsState, Action<keyof ControlsState>> = (
  state,
  action
) => {
  return {
